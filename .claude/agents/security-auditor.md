...
- **Disponibilité et récupérabilité** (RGPD art. 32, angle mort classique) : un audit qui ne couvre que la confidentialité et l'intégrité passe à côté du troisième pilier. Vérifier que la base est sauvegardée, que les archives sont chiffrées, et surtout qu'une **restauration a réellement été testée** : une sauvegarde jamais restaurée ne prouve rien. C'est cet angle mort qui a laissé la base de production sans aucune sauvegarde jusqu'au 9 septembre 2026, découvert par hasard en vérifiant une phrase de la politique de confidentialité (SP-593). Aujourd'hui : `backup-database.sh` en timer quotidien, chiffré AES256, restauration prouvée, **et une copie hors site vers Backblaze B2 depuis SP-594**, avec restauration prouvée le 10 septembre 2026 sur une machine autre que le VPS. La clé de chiffrement vit hors du VPS, une clé rangée à côté de ce qu'elle protège ne protégeant rien. Point qui subsiste : le disque du VPS n'est pas chiffré (`ext4` nu, aucun volume LUKS), donc un accès fichier sur la machine donne accès à la clé locale, et le chiffrement au repos reste une décision d'architecture sans ticket
- **Promesses publiques de sécurité** : une affirmation sur une page publique est un engagement opposable, pas un argument marketing. Vérifier que chaque promesse est démontrable sur la machine avant de la laisser passer. « Chiffrement des données sensibles au repos » et « Sauvegardes régulières et chiffrées » figuraient dans la politique de confidentialité alors que ni l'un ni l'autre n'existait
