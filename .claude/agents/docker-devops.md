...
- **`ufw` ne filtre pas les ports publiés par Docker** : Docker écrit ses règles DNAT dans `iptables` en amont de la chaîne d'`ufw`, donc `-p 3000:3000` publie sur toutes les interfaces et répond depuis Internet alors qu'`ufw status` dit le port fermé. Tout port qui n'a pas à être joignable de l'extérieur se publie en `127.0.0.1:3000:3000`. **Se vérifier depuis une autre machine**, jamais depuis le VPS où `curl localhost:3000` répond toujours 200 sans rien prouver : `curl http://51.77.146.72:<port>` de l'extérieur, ou `ss -tlnp | grep -E '0\.0\.0\.0|\[::\]'` côté serveur. SP-583, 8 septembre 2026, l'application et Umami servaient l'authentification hors TLS sur les ports 3000 et 3001. Avant de basculer un port, lire la cible du `proxy_pass` ou du bloc `upstream` : ils visent `localhost` ici, ce qui ne vaut `127.0.0.1` que parce que `/etc/hosts` nomme `::1` `ip6-localhost`
...
- **Copie hors site** : `scripts/ops/sync-backups-offsite.sh`, timer `smartplanning-backup-offsite.timer` à 04:10 UTC, soit 50 minutes après la sauvegarde locale pour absorber son `RandomizedDelaySec`. Destination Backblaze B2, fournisseur volontairement distinct d'OVH. **Il y a donc DEUX timers de sauvegarde**, ne pas n'en voir qu'un. API native B2 et non S3, qui imposerait une signature AWS v4 en shell ; aucun outil installé, `curl`, `jq` et `sha1sum` suffisent. Identifiants dans `/etc/smartplanning/b2.conf` (0600, root), jamais dans le `.env` que le CD recopie. Le chiffrement côté serveur du bucket reste **désactivé** : les fichiers partent déjà chiffrés avec notre passphrase, activer celui de B2 ajouterait une couche dont nous n'aurions pas la clé (SP-594)
