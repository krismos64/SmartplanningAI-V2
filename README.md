...
- **Webhooks Stripe enrichis** (`/api/webhooks/stripe`) :
  - `checkout.session.completed` → `SubscriptionConfirmedEmail` + notification SSE SYSTEM_ADMIN (SUCCESS)
  - `invoice.payment_failed` → `PaymentFailedEmail` + notification SSE SYSTEM_ADMIN (WARNING, HIGH)
  - `customer.subscription.deleted` → `SubscriptionCanceledEmail` + notification SSE SYSTEM_ADMIN (WARNING, HIGH)
  - `invoice.paid` → `InvoiceEmail` avec détails facture + notification SSE SYSTEM_ADMIN (SUCCESS, LOW)
...
- **SP-476 — Notifications SSE temps réel SYSTEM_ADMIN** : Extension du système SSE existant pour 4 types de notifications admin : `NEW_COMPANY_REGISTERED` (inscription), `SUBSCRIPTION_PAST_DUE` (paiement échoué), `SUBSCRIPTION_CANCELED` (résiliation), `TRIAL_EXPIRED` (fin d'essai). Factory `createAdminNotification()` avec pattern fire-and-forget. Migration Prisma `Notification.companyId` optionnel. **Intégrations complètes** : toutes les factory functions SSE sont branchées — planning `deleted` (deleteSchedule + deleteScheduleGroup avec lookup userId), congé `cancelled` (notification managers de l'équipe), incident `created`/`updated` (notification employé sujet si visibility=ALL), impersonation stop (notification admin avec durée), Stripe checkout/subscription.deleted/invoice.paid (3 notifications admin fire-and-forget). 7 tests unitaires
